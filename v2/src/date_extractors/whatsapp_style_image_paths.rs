...
/// Example file paths:
///   * /storage/emulated/0/DCIM/Camera/IMG-20250127-WA0006POSTFIX.jpg
pub fn get_date_from_whatsapp_filepath_regex(
  _file_path: &Path,
  file_name: &str,
) -> Option<(NaiveDateTime, DateConfidence)> {
  let re = Regex::new(r"IMG-(\d{4})(\d{2})(\d{2})-WA\d+").unwrap();
...
pub mod test {
  ...
  pub static TESTS_WHATSAPP_FILEPATH: LazyLock<[TestCase; 2]> = LazyLock::new(|| {
    [
      TestCase {
